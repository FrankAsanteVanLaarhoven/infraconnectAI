...
export function RevenueVictoryHub() {
  ...
  return (
    <div className="w-full h-full bg-[#020202] border border-indigo-500/20 rounded-3xl p-8 flex flex-col font-mono relative overflow-hidden group">
       ...
       <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6 bg-black/40 backdrop-blur-md relative z-10">
          ...
          <div className="flex gap-12 text-right">
             <div className="flex flex-col">
                ...
                <span className="text-3xl font-black text-slate-300 tracking-tighter tabular-nums">${totalROI}M</span>
             </div>
             <div className="flex flex-col">
       ...
       <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-0 relative z-10">
             ...
             <AnimatePresence>
                {closures.map((c, i) => (
                   <motion.div 
                      ...
                      className="p-5 bg-white/[0.02] border border-white/5 rounded-none flex items-center justify-between group hover:bg-white/[0.04] hover:border-indigo-500/30 transition-all cursor-pointer"
                   >
                      <div className="flex items-center gap-6">
                         <div className={`p-3 rounded-sm ${c.status === 'TARGET_CLEARED' ? 'bg-slate-800 border border-slate-700' : 'bg-indigo-500/10 border border-indigo-500/20'}`}>
                            {c.status === 'TARGET_CLEARED' ? <ShieldCheck className="w-5 h-5 text-slate-300" /> : <Zap className="w-5 h-5 text-indigo-400" />}
                         </div>
                         <div>
                            <div className="flex items-center gap-2 mb-1">
                               <h3 className="text-xs font-black text-white uppercase tracking-wider">{c.company}</h3>
                               <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${c.status === 'TARGET_CLEARED' ? 'bg-slate-800 text-slate-300' : 'bg-indigo-950 text-indigo-500'}`}>
                                  {c.status}
                               </span>
                            </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
             ...
             {/* Swarm Status */}
             <div className="p-6 bg-slate-900/40 border border-white/5 rounded-none space-y-4">
                ...
                <div className="space-y-4">
                   ...
                   <div className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] font-black text-slate-300">+18.4%</span>
                   </div>
             ...
             <div className="p-6 bg-indigo-600/5 border border-indigo-500/20 rounded-none space-y-4 group">
                ...
                <button className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-black text-[10px] font-black uppercase tracking-widest rounded-sm transition-all">
                   Initiate Re-Strike
                </button>
             </div>
