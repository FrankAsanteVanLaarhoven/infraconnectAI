...
export function ConnectorNode({ data }: { data: { label: string; status?: string } }) {
  ...
  return (
    <div className="w-64 rounded-lg bg-[#0a0b0c] border border-slate-800 shadow-2xl relative overflow-hidden group">
      ...
      <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between bg-black/40">
        ...
        {isConnected && <Activity className="w-3 h-3 text-green-500 animate-pulse" />}
      </div>
      ...
      <div className="px-4 py-4 relative">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className={`text-[10px] uppercase font-mono tracking-wider ${isConnected ? 'text-green-400' : 'text-yellow-500'}`}>
        ...
        <div className="h-1 w-full bg-slate-900 rounded overflow-hidden">
          <div className={`h-full bg-green-500 transition-all ${isConnected ? 'w-full animate-pulse' : 'w-1/3 opacity-50'}`} />
        </div>
      ...
      <Handle type="source" position={Position.Right} className="w-3 h-3 bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.8)] border-none right-[-6px]" />
    </div>
  );
}
