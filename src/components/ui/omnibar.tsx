      ...
      {open && (
        ...
        >
          <motion.div 
            ...
            className="w-full max-w-2xl bg-[#0a0b0c] border border-slate-700/80 rounded-sm overflow-hidden flex flex-col"
          >
            {/* Input Header */}
            <div className="flex items-center px-4 py-3 border-b border-slate-800 bg-[#111214]">
              <Sparkles className={`w-5 h-5 mr-3 transition-colors ${topologyPreview ? 'text-slate-300' : 'text-slate-500'}`} />
              <Command.Input 
                autoFocus
                ...
                {topologyPreview ? (
                  <div className="flex items-center gap-4 w-full px-8 animate-in fade-in zoom-in-95 duration-500">
                    {topologyPreview.nodes.map((n, i) => (
                      <React.Fragment key={n.id}>
                        <div className={`flex flex-col items-center gap-3 w-1/3 p-4 rounded-sm border border-slate-800 bg-[#111] shadow-[0_4px_20px_rgba(0,0,0,0.5)] ${i === 0 ? 'border-l-blue-500/50' : i === 2 ? 'border-b-purple-500/50' : ''}`}>
                          ...
                          {n.type === 'ai' && <Cpu className="w-8 h-8 text-slate-400" />}
                  ...
                  <div className="text-slate-600 text-sm flex flex-col items-center gap-3">
                    {query.length > 0 && !query.startsWith('>') && !query.startsWith('!') ? (
                      <span className="">Analyzing semantic intent...</span>
                    ) : query.startsWith('>') ? (
                      <div className="flex flex-col gap-2 w-full max-w-sm text-left p-4 bg-[#0B0D0F] border border-[#1A1D21] rounded">
                         <div className="text-[#4CC9F0] font-mono text-[10px] tracking-widest font-black uppercase flex items-center gap-2 mb-1">
                            <TerminalSquare className="w-4 h-4" /> COMMAND MODE
                         </div>
                      ...
                      <div className="flex flex-col gap-2 w-full max-w-sm text-left p-4 bg-[#2e1065]/20 border border-slate-800 rounded">
                         <div className="text-slate-400 font-mono text-[10px] tracking-widest font-black uppercase flex items-center gap-2 mb-1">
                            <Cpu className="w-4 h-4" /> AUTONOMOUS AGENT MODE
                         </div>
                         <div className="text-[11px] text-slate-400 font-mono">
                            <span className="text-slate-400">Workflow:</span> {query.slice(1).trim() || '...'}
                         </div>
                      </div>
                    ) : (
                      <>
                        ...
                        <div className="flex flex-wrap justify-center gap-2 mt-2 max-w-lg">
                           ...
                           <span className="text-[10px] bg-[#0a0a0a] border border-[#1F1F1F] px-2 py-1 flex items-center gap-1.5 rounded text-[#9AA4AF] cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setQuery("! stabilize fleet")}><span className="text-slate-400">AGENT</span> {"! stabilize fleet"}</span>
