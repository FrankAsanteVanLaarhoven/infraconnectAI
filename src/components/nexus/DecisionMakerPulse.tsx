...
export function DecisionMakerPulse() {
    ...
    return (
        <div className="w-full h-full bg-[#020617]/80 backdrop-blur-[40px] border border-slate-700 rounded-3xl p-6 flex flex-col font-mono relative overflow-hidden group">
            {/* Header */}
            ...
            <div className="flex-1 grid grid-cols-1 gap-4 overflow-y-auto custom-scrollbar pr-2">
                {execs.map((exec, i) => (
                    <motion.div
                        ...
                        className="p-4 bg-black/40 border border-white/5 rounded-none flex items-center justify-between group/exec hover:border-slate-700 transition-all cursor-crosshair"
                    >
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-sm bg-slate-900 flex items-center justify-center border border-white/10 group-hover/exec:border-slate-700 transition-colors">
                                <Briefcase className="w-4 h-4 text-slate-400 group-hover/exec:text-slate-300" />
                            </div>
                        ...
                        <div className="flex flex-col items-end gap-1">
                            <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${
                                exec.sentiment === 'BULLISH' ? 'text-slate-300 bg-slate-800' :
            ...
            {/* Bottom Status */}
            <div className="mt-6 p-4 bg-slate-800 border border-slate-700 rounded-none flex items-center justify-between">
                ...
                <div className="flex gap-1 h-3 items-end">
                    {[0.4, 0.7, 0.2, 0.9, 0.5, 0.8].map((h, i) => (
                        <motion.div 
                            ...
                            className="w-1 bg-slate-800 rounded-t-sm"
                        />
                    ))}
                </div>
            </div>
