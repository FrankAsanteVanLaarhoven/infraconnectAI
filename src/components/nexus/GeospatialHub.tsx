        ...
        <div className="w-full h-full bg-slate-950/90 backdrop-blur-3xl border border-slate-800 rounded-2xl p-6 flex flex-col font-mono relative overflow-hidden group">
            ...
            <div className="mb-6 relative">
                ...
                <input 
                    ...
                    className="w-full bg-slate-900/50 border border-slate-800 rounded-xl py-4 pl-32 pr-4 text-[11px] text-white focus:outline-none focus:border-emerald-500/50 transition-all font-mono placeholder:text-slate-700 uppercase"
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
                <div className="lg:col-span-2 relative bg-slate-900/30 border border-slate-800/50 rounded-2xl overflow-hidden group/map">
                    {/* Simulated Map Grid */}
                    ...
                    {MOCK_BUSINESSES.map((b) => (
                        <motion.div 
                            ...
                            className={`absolute w-4 h-4 rounded-full border-2 cursor-pointer z-10 ${
                                selectedEntity === b.id 
                                ? 'bg-emerald-500 border-white shadow-[0_0_20px_#10b981]' 
                                : 'bg-slate-950 border-emerald-500/50 hover:border-emerald-500'
                            }`}
                        >
                            <AnimatePresence>
                                {selectedEntity === b.id && (
                                    <motion.div 
                                        ...
                                        className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-slate-950 border border-slate-800 p-3 rounded-lg w-48 shadow-2xl z-20"
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <span className="text-[9px] font-black text-white uppercase">{b.name}</span>
                                            <span className="text-[7px] text-emerald-500 font-bold">{b.growth}</span>
                                        </div>
                                        <p className="text-[7px] text-slate-500 uppercase italic mb-2">{b.location}</p>
                                        <Button className="w-full py-1 h-auto text-[7px] bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all">
                                            Ingest to Ontology
                                        </Button>
                                    </motion.div>
                                )}
                ...
                <div className="flex flex-col gap-4">
                            ...
                            {MOCK_BUSINESSES.map(b => (
                                <div 
                                    ...
                                    className={`p-2 rounded border cursor-pointer transition-all ${
                                        selectedEntity === b.id ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-slate-950 border-slate-800/50 hover:bg-slate-900'
                                    }`}
                                >
                    ...
                    <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl space-y-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] text-emerald-500 uppercase font-black tracking-widest">Strategic Pulse</span>
                        </div>
                        ...
                        <Button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase text-[9px] py-4 rounded-lg tracking-widest flex items-center justify-center gap-2">
