...
export function TerafabComputeLens() {
    ...
    return (
        <div className="w-full h-full bg-[#020617]/80 backdrop-blur-[40px] border border-cyan-500/20 rounded-3xl p-6 flex flex-col font-mono relative overflow-hidden group shadow-[0_0_60px_rgba(34,211,238,0.05)]">
            {/* Header */}
            <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6 relative z-10">
                ...
                <div className="bg-black/60 px-3 py-1 border border-cyan-500/20 rounded-lg">
                    <span className="text-[10px] text-cyan-400 font-black animate-pulse">COMPUTE_ACTIVE</span>
                </div>
            </div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto custom-scrollbar pr-2 relative z-10">
                {nodes.map((node, i) => (
                    <motion.div
                        ...
                        className="p-5 bg-slate-900/60 border border-white/5 rounded-2xl flex flex-col gap-4 group/node hover:border-cyan-500/30 transition-all"
                    >
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                <div className={`w-2 h-2 rounded-full ${
                                    node.health === 'OPTIMAL' ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' :
                                    node.health === 'NOMINAL' ? 'bg-cyan-500 shadow-[0_0_10px_#22d3ee]' : 'bg-red-500 shadow-[0_0_10px_#ef4444]'
                                }`} />
