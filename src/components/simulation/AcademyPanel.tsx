...
export function AcademyPanel({ onClose }: { onClose: () => void }) {
  return (
    <div className="absolute inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-8">
      <div className="bg-[#0a0a0a] border border-white/10 rounded-xl w-full max-w-4xl h-full max-h-[80vh] flex flex-col shadow-2xl overflow-hidden relative pointer-events-auto">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6">
           <div className="grid grid-cols-2 gap-6">
              {ACADEMY_MANIFEST.map((course, idx) => (
                 <div key={idx} className="bg-black/50 border border-emerald-500/10 rounded-lg p-4">
                    <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-2">
                       <h3 className="text-emerald-400 font-medium font-mono text-sm flex items-center gap-2">
                           <PlayCircle className="h-4 w-4" />
                           {course.module}
                       </h3>
                       <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded">INGESTED</span>
                    </div>
