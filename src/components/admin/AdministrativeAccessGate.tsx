    ...
    <div className="flex flex-col gap-8 p-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="bg-gradient-to-br from-amber-500/10 to-transparent border border-white/5 p-5 rounded-3xl flex items-center gap-5 shadow-2xl relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-16 h-16 bg-amber-500/5 blur-[40px] rounded-full" />
            <div className="w-12 h-12 bg-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.4)] rounded-2xl flex items-center justify-center text-black relative z-10 transition-transform group-hover:scale-110">
         ...
         <div className="bg-slate-900/40 border border-white/5 p-5 rounded-3xl flex items-center gap-5 shadow-inner backdrop-blur-md">
            <div className="w-12 h-12 bg-slate-800/80 rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-cyan-400 transition-colors">
         ...
         <div className="bg-slate-900/40 border border-white/5 p-5 rounded-3xl flex items-center gap-5 shadow-inner backdrop-blur-md group">
            <div className="w-12 h-12 bg-slate-800/80 rounded-2xl flex items-center justify-center text-slate-400">
               <RotateCcw className="w-7 h-7 text-green-500/80 animate-[spin_10s_linear_infinite]" />
            </div>
            <div>
      ...
      <div className="bg-[#020202] border border-white/5 rounded-3xl overflow-hidden flex flex-col min-h-[580px] shadow-[0_40px_80px_rgba(0,0,0,0.6)] relative">
         ...
         <div className="flex border-b border-white/5 px-8 bg-black/40 backdrop-blur-3xl sticky top-0 z-20">
            {[
              ...
              >
                <tab.icon className={`w-4 h-4 transition-transform ${activeTab === tab.id ? 'scale-110 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]' : ''}`} />
                {tab.label}
                {activeTab === tab.id && (
                  <motion.div layoutId="iamTab" className="absolute bottom-0 left-0 right-0 h-[2.5px] bg-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.8)]" />
                )}
              </button>
            ))}
         </div>
            ...
            <AnimatePresence mode="wait">
                  ...
                  >
                     <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-3">
                           <div className="w-2 h-2 rounded-full bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,1)]" />
                           <h4 className="text-xs font-black text-white uppercase tracking-[0.3em]">AI Chain of Thought</h4>
                        </div>
                        <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest border border-slate-800 px-3 py-1 rounded-full">REAL-TIME INFERENCE</span>
                     </div>
                        ...
                        {intents.map((intent) => (
                           ...
                           >
                              ...
                              <div className="flex items-start gap-4">
                                 <div className="mt-1 p-2.5 bg-amber-500/10 rounded-xl border border-amber-500/20 text-amber-500 group-hover/intent:bg-amber-500 group-hover/intent:text-black transition-colors">
                              ...
                              <div className="mt-4 h-[1px] bg-slate-900 rounded-full overflow-hidden">
                                 <motion.div 
                                   initial={{ width: 0 }}
               ...
               {activeTab === 'principals' && (
                  ...
                  >
                     <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-3">
                           <div className="w-2.5 h-2.5 rounded bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]" />
                           <h4 className="text-xs font-black text-white uppercase tracking-[0.3em]">Operational Unit Inventory</h4>
                        </div>
                        <button className="px-5 py-2.5 bg-amber-500 text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-400 transition-all flex items-center gap-3 shadow-2xl">
                          ...
                          { name: 'Sentinel_Gateway_X', role: 'Edge_Logic', clearance: 'L2_EDGE', status: 'STANDBY', val: 0 }
                           ...
                           <div key={user.name} className="bg-slate-900/20 border border-white/5 p-5 rounded-3xl flex items-center justify-between group transition-all hover:bg-white/[0.03]">
                              <div className="flex items-center gap-5">
                                 <div className="w-12 h-12 rounded-2xl bg-slate-800/50 flex items-center justify-center text-slate-500 group-hover:bg-amber-500 group-hover:text-black transition-all duration-500">
                              ...
                              <div className="flex flex-col items-end gap-3">
                                 <div className={`px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest border ${user.status === 'SYNCHRONIZED' ? 'bg-green-500/10 text-green-500 border-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.1)]' : 'bg-slate-800 text-slate-500 border-slate-700'}`}>
                                    {user.status}
                                 </div>
                                 <div className="flex items-center gap-2">
                                    <div className="w-20 h-1 bg-slate-800/40 rounded-full overflow-hidden border border-white/5">
                          ...
                          { title: 'Neural Hardening', status: 'LEVEL 9', desc: 'Sub-pixel encryption for all administrative visual feeds.' }
                           ...
                           <div key={pol.title} className="bg-white/[0.01] border border-white/5 p-6 rounded-3xl flex flex-col gap-4 hover:bg-white/[0.03] hover:border-white/10 transition-all group/pol relative overflow-hidden">
                              <div className="flex justify-between items-start z-10">
                                 ...
                                 <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-slate-700 group-hover/pol:text-amber-500 transition-colors">
