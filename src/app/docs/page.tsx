...
const parseMarkdownToJSX = (text: string) => {
  ...
  return lines.map((line, idx) => {
    if (line.startsWith('# ')) {
      return <h1 key={idx} className="text-3xl font-black text-white tracking-widest uppercase mb-6 mt-8 flex items-center gap-3"><TerminalSquare className="text-zinc-400 w-6 h-6" />{line.replace('# ', '')}</h1>;
    }
    if (line.startsWith('### ')) {
      return <h3 key={idx} className="text-sm font-black text-zinc-300 tracking-widest uppercase mb-4 mt-8 border-b border-zinc-800 pb-2">{line.replace('### ', '')}</h3>;
    }
    if (line.startsWith('> **Caution**')) {
      return (
         <div key={idx} className="bg-red-950/20 border border-red-500/30 p-4 rounded-sm my-6 flex gap-3 text-red-200">
    ...
    if (line.startsWith('- **')) {
       ...
       return (
          <li key={idx} className="text-slate-300 font-mono text-xs mb-3 flex items-start gap-2">
             <Zap className="w-3 h-3 text-slate-300 mt-0.5 shrink-0" />
...
export default function AppDocsPage() {
  ...
  return (
    <div className="flex h-full w-full bg-[#050607] text-white overflow-hidden relative">
      ...
      {/* Background Ambience */}
      <div className="absolute top-[-10%] right-[-5%] w-[800px] h-[800px] bg-zinc-800/10 rounded-sm blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] left-[-10%] w-[600px] h-[600px] bg-slate-800/10 rounded-sm blur-[100px] pointer-events-none" />

      {/* Docs Sidebar */}
      <aside className="w-72 border-r border-white/5 bg-black/40 backdrop-blur-md flex flex-col h-full z-10 relative">
         <div className="p-6 border-b border-white/5">
            <PremiumBackButton href="/dashboard" label="Platform" className="mb-8" />
            <div className="flex items-center gap-3 mb-6">
                <div className="w-8 h-8 rounded bg-zinc-800/30 border border-zinc-700 flex items-center justify-center">
                   <BookOpen className="w-4 h-4 text-zinc-300" />
                </div>
                <div>
                   <h2 className="text-[12px] font-black uppercase tracking-[0.2em] text-white">Intelligence</h2>
                   <p className="text-[9px] text-zinc-500 font-mono tracking-widest">KNOWLEDGE BASE v2.4</p>
                </div>
            </div>
            ...
            <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-sm pl-8 pr-3 py-2 text-xs font-mono focus:outline-none focus:border-zinc-500/50 text-white placeholder:text-slate-600 transition-colors"
                />
            </div>
         </div>
                         ...
                         return (
                           <li key={section.id}>
                              <button 
                                ...
                                className={cn(
                                   "w-full text-left px-3 py-2 rounded-sm font-mono text-[11px] transition-all flex justify-between items-center",
                                   isActive 
         ...
         <div className="p-4 border-t border-white/5 bg-white/[0.02]">
            <div className="flex items-center justify-between text-[9px] font-mono tracking-widest uppercase">
               ...
               <span className="text-slate-300 flex items-center gap-1"><div className="w-1.5 h-1.5 bg-slate-800 rounded-sm" /> INDEXED</span>
            </div>
         </div>
      </aside>
      ...
      <main className="flex-1 h-full overflow-y-auto custom-scrollbar relative z-10 scroll-smooth">
         <div className="max-w-4xl mx-auto px-12 py-16">
            ...
            <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-slate-500 mb-8">
                ...
                <span>/</span>
                <span className="text-zinc-400">{DocumentationManifest.find(c => c.sections.some(s => s.id === activeId))?.category}</span>
                <span>/</span>
                <span className="text-white">{activeContent?.title}</span>
            </div>

            <div className="glass-panel p-10 rounded-none bg-black/20 border border-white/5 shadow-2xl backdrop-blur-xl relative overflow-hidden">
               {/* Accent line */}
