...
export default function EnvironmentPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-slate-300 font-mono flex flex-col overflow-hidden selection:bg-emerald-500/30">
       ...
       <header className="h-20 border-b border-white/5 bg-white/[0.02] flex items-center justify-between px-8 relative z-50">
          <div className="flex items-center gap-6">
             <div className="flex items-center gap-3">
                <div className="p-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg shadow-[0_0_20px_rgba(16,185,129,0.1)]">
             ...
             <div className="flex flex-col">
                ...
                <span className={`text-sm font-black tracking-widest transition-colors ${planetaryRisk > 0.5 ? 'text-red-500' : 'text-emerald-500'}`}>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      {metrics.map(metric => (
                         <button 
                           ...
                           className={`p-6 bg-white/[0.02] border transition-all rounded-2xl flex flex-col gap-4 text-left group ${
                              selectedMetric?.id === metric.id ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-white/5 hover:border-white/10'
                           }`}
                         >
                            <div className="flex justify-between items-start">
                               ...
                               <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${
                                  metric.status === 'CRITICAL' ? 'bg-red-500 text-black animate-pulse' : 'bg-emerald-950 text-emerald-400'
                               }`}>
                                  {metric.status}
                               </span>
                            </div>
                            ...
                            <div className="flex items-end justify-between">
                               ...
                               <div className={`flex items-center gap-1 text-[10px] font-black ${metric.trend === 'UP' ? 'text-red-500' : 'text-emerald-500'}`}>
             ...
             <div className="p-8 bg-black/40 space-y-8">
                ...
                {selectedMetric && (
                   <div className="space-y-6">
                      <div className="p-6 bg-red-950/10 border border-red-500/20 rounded-2xl relative overflow-hidden group">
                      ...
                      <div className="h-[250px] bg-slate-900/10 border border-white/5 rounded-2xl p-4">
                ...
                <div className="space-y-3">
                   {[
                     ...
                     { label: 'Infrastructure Hardening', impact: '+15.2% Resilience' }
                      ...
                      <div key={i} className="p-4 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between group hover:border-cyan-500/30 transition-all cursor-pointer">
                         <div>
                            <span className="text-[9px] font-black text-white uppercase tracking-wider block mb-1">{item.label}</span>
                            <span className="text-[8px] text-emerald-500 font-bold uppercase">{item.impact}</span>
                         </div>
       ...
       <footer className="h-8 border-t border-white/5 bg-black flex items-center justify-between px-6 text-[8px] font-black text-slate-600 uppercase tracking-widest">
          <div className="flex items-center gap-4">
             <span className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> PLANETARY_OS_SYNC: EARTH-V20</span>
             <span>ATMOS_FIDELITY: 99.4%</span>
             <span>CIVILIZATIONAL_PULSE: STABLE_BETA</span>
          </div>
          <div className="flex items-center gap-4">
             <span className="text-emerald-900 underline">ES_LOGS:ENV01</span>
