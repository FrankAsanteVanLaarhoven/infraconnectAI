  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center text-center space-y-12">
      <div className="space-y-4">
       ...
       <div className="w-full max-w-2xl bg-black border border-slate-800 p-10 rounded-sm relative overflow-hidden group">
         <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-blue-500/50 to-transparent" />
         <TrendingUp className="w-12 h-12 text-blue-500 mx-auto mb-8 drop-" />
         {stage >= 1 ? (
           <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2 mb-10">
             ...
             <div className="text-5xl font-black text-blue-400 font-mono drop-">£{roi.toLocaleString()}<span className="text-3xl text-slate-600">/mo</span></div>
           ...
           <div className="h-[120px] flex items-center justify-center text-xs text-slate-600 font-mono uppercase tracking-widest">Calculating systemic ROI...</div>
         )}
         {stage >= 2 && (
...
export default function EpicTheatre() {
  ...
  const { lastIncident, lastTelemetry } = useFleetStream();
  ...
  useLiveEvents((event) => {
      ...
      else if (event.type === 'TELEMETRY') setAct(prev => prev <= 2 ? 2 : prev);
  ...
  useEffect(() => {
    ...
    const runDemo = async () => {
      ...
      const stages = [
        { act: 2, delay: 1000, type: 'NODE_CONNECTED', log: '[NODE_CONNECTED] EDGE-001' },
        { act: 3, delay: 2500, type: 'ANOMALY_DETECTED' },
        { act: 4, delay: 2500, type: 'DEPLOY_STARTED', log: '[DEPLOY_STARTED] Model injection initialized...' },
        { act: 5, delay: 2500, type: 'DEPLOY_COMPLETE', terminal: 'Execute: Trust Validation Phase' },
        { act: 6, delay: 2500, type: 'TRUST_LOCK' },
        { act: 7, delay: 4000, type: 'DEMO_FINISH' }
      ];
      ...
      for (const stage of stages) {
        ...
        setIsLive(true);
        if (stage.type !== 'DEMO_FINISH') {
          triggerEventBeat(stage.type);
          playCinematicSound(stage.type);
        }
        
        setAct(stage.act);
        if (stage.log) setLogs(prev => [...prev, stage.log]);
        if (stage.terminal) setTerminalText(stage.terminal);
      }
    };
    ...
    <motion.div animate={controls} className={`relative min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center font-mono overflow-hidden transition-all duration-300`}>
      ...
      <div className="absolute top-6 left-6 z-[60] flex items-center gap-6">
        ...
        <div className="flex flex-col font-mono">
          ...
          <p className="text-[10px] text-slate-300 uppercase tracking-widest font-bold flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-sm bg-slate-800" />
            Connected
          </p>
        </div>
      </div>
      ...
      {act === 2 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 1.2, ease: [0.16, 1, 0.3, 1] } }} className="w-full max-w-6xl px-8 grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
           <div className="space-y-8">
              ...
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] p-6 rounded-sm">
           ...
           <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-sm blur opacity-25 group-hover:opacity-40 transition duration-1000 group-hover:duration-200"></div>
              <div className="relative h-[500px] overflow-hidden rounded-sm border border-slate-800/50">
      ...
      {act === 3 && (
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0, transition: { duration: 1.2, ease: [0.16, 1, 0.3, 1] } }} className="w-full h-full max-w-7xl mx-auto flex flex-col items-center p-8 relative z-10">
           ...
           <div className="flex-1 w-full border border-white/10 rounded-none overflow-hidden bg-white/5 backdrop-blur-2xl shadow-[0_10px_40px_rgba(0,0,0,0.4)] relative group">
              <SwarmOrchestrator />
              
              {/* Overlay Prompt */}
              <div className="absolute bottom-12 right-12 z-[80]">
                 <button className="group bg-white/5 backdrop-blur-2xl border border-white/10 text-white/50 px-8 py-4 rounded-sm text-xs flex items-center gap-3 transition-all shadow-[0_10px_40px_rgba(0,0,0,0.4)]">
      ...
      {act === 4 && (
        <div className="w-full max-w-4xl p-8 bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] rounded-sm relative">
      ...
      {act === 5 && (
        <motion.div initial={{ opacity: 0, filter: "blur(12px)" }} animate={{ opacity: 1, filter: "blur(0px)", transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1] } }} className="w-full max-w-6xl p-8">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-[500px]">
             <div className="bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] p-8 rounded-sm flex flex-col justify-between">
                <div>
                   <h3 className="text-xs uppercase text-white/50 tracking-widest mb-6">Live Command Log</h3>
                   <div className="text-lg text-white font-mono mb-6">{terminalText}<span className="">_</span></div>
                </div>
                {lastTelemetry && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1] } }} className="bg-white/5 border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] p-6 rounded-sm mt-auto">
              ...
              <div className="bg-white/5 backdrop-blur-2xl border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.4)] rounded-sm overflow-hidden flex flex-col">
