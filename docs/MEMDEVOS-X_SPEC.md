---
title: "MEMDEVOS-X: Governed Autonomy for Embodied Coding Agents"
version: "1.0.0"
...
**MEMDEVOS-X** is a policy-governed autonomous agent operating system. It shifts the benchmark for autonomous systems from pure capability (can it code?) to **Governed Autonomy** (can it act safely, recover autonomously, and collaborate with humans under strict policy constraints over long horizons?).
...
MEMDEVOS-X must prove its thesis via ablations on standard CaP-style target sets.

## 3.1 Experimental Systems
- **System A:** Baseline CaP-Agent (Code Generation alone).
- **System B:** CaP-Agent + MEMDEVOS Memory (Governance).
- **System C:** CaP-Agent + MEMDEVOS + NemoClaw (Secure Execution).
- **System D (MEMDEVOS-X):** CaP-Agent + MEMDEVOS + NemoClaw + PersonaPlex.
